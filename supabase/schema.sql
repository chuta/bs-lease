...
-- Apartment units (admin-controlled inventory; public EOI reads availability via Netlify service role only)
...
-- No anon policies: inventory is edited only by authenticated admins; public reads go through Netlify (service role).
