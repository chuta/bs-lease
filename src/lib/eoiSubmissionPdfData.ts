...
export function submissionDetailToPdfData(detail: EoiSubmissionDetail): EoiPdfData {
  return {
    ...
    facebookHandle: (detail.facebook_handle ?? "").trim(),
    xHandle: (detail.x_handle ?? "").trim(),
    instagramHandle: (detail.instagram_handle ?? "").trim(),
    linkedinHandle: (detail.linkedin_handle ?? "").trim(),
    nin: detail.nin,
